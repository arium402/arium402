let selectedContactMethod = 'phone';
        ...
        document.getElementById('findPasswordForm').addEventListener('submit', function(e) {
            ...
            // 기본 정보 검증
            if (!studentId) {
                showError('studentIdError', '학번을 입력해주세요.');
                hasError = true;
            }
            ...
            if (selectedContactMethod === 'phone') {
                contactValue = document.getElementById('studentPhone').value.trim();
                if (!contactValue) {
                ...
                contactValue = document.getElementById('studentEmail').value.trim();
                if (!contactValue) {
            ...
            // 서버로 데이터 전송
            sendTemporaryPassword(studentId, name, selectedContactMethod, contactValue);
        });
        
        function sendTemporaryPassword(studentId, name, method, contact) {
            // 로딩 표시
