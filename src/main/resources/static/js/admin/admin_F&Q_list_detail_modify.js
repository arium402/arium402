   ...
   // 수정할 페이지의 기존 데이터 (URL 파라미터에서 가져옴)
   const existingPageData = {
       category: 'extracurricular',
       subCategory: 'program-apply',
       faqItems: [
           { id: 1, question: "비교과 프로그램은 어떻게 신청하나요?", answer: "비교과 프로그램 신청은 학교 홈페이지 로그인 후 '비교과' 메뉴에서 신청하실 수 있습니다. 신청 기간은 각 프로그램별로 상이하므로 공지사항을 확인해주세요." },
           { id: 2, question: "비교과 프로그램 참여 시 마일리지를 받을 수 있나요?", answer: "네, 비교과 프로그램 참여 시 마일리지가 지급됩니다. 프로그램 종류에 따라 지급되는 마일리지가 다르며, 수료증 발급 시에도 추가 마일리지가 지급됩니다." },
           { id: 3, question: "비교과 프로그램 수강료는 얼마인가요?", answer: "대부분의 비교과 프로그램은 무료로 제공됩니다. 단, 자격증 취득 과정이나 특별 프로그램의 경우 별도의 수강료가 있을 수 있으니 각 프로그램 안내를 확인해주세요." }
       ]
   };
   ...
   function loadExistingData() {
       // 대분류 설정
       document.getElementById('mainCategory').value = existingPageData.category;
       
       // 소분류 옵션 업데이트
       const subCategorySelect = document.getElementById('subCategory');
       subCategorySelect.innerHTML = '<option value="">선택하세요</option>';
       
       if (subCategories[existingPageData.category]) {
           subCategories[existingPageData.category].forEach(sub => {
               const option = document.createElement('option');
               option.value = sub.value;
               option.textContent = sub.text;
               subCategorySelect.appendChild(option);
       ...
       // 소분류 설정
       document.getElementById('subCategory').value = existingPageData.subCategory;
       
       // 기존 Q&A 아이템들 로드
       const container = document.getElementById('qaListContainer');
       container.innerHTML = '';
       
       existingPageData.faqItems.forEach((item, index) => {
           const qaItem = document.createElement('div');
           qaItem.className = 'qa-item';
           qaItem.setAttribute('data-index', index);
           qaItem.setAttribute('draggable', 'true');
           ...
           qaItem.innerHTML = `
               ...
               <div class="qa-order-number">${index + 1}</div>
               <div class="qa-content">
                   <div class="qa-question">
                       <span class="qa-label">Q:</span>
                       <div class="qa-question-content">${item.question}</div>
                       <div class="qa-controls">
                           <button class="btn-remove" onclick="removeQAItem(${index})">
           ...
           setupDragEvents(qaItem);
       });
       
       qaCounter = existingPageData.faqItems.length;
       orderCounter = existingPageData.faqItems.length;
   }
